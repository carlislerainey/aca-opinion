...
# load data
d <- read.spss("Data/Kaiser/kaiser_2013_11.por")
...
table(d$QD8C)
tea.party[d$QD8C=="Yes, supporter of Tea Party movement"] <- 1
tea.party[d$QD8C=="No, not a supporter of Tea Party movement"] <- 0
...
new.data$month <- factor("November")
new.data$year <- factor("2013")
new.data$polling.org <- factor("Kaiser")

# write csv file
write.csv(new.data, "Data/Cleaned_Poll_Data/kaiser_2013_11.csv", row.names = FALSE)
